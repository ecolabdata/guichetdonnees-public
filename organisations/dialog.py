...
class Ckan:
    ...
    ----------
    intranet : bool
        True pour une connexion depuis le RIE. False
        par défaut. Détermine le proxy à utiliser.
    ...
    Notes
    -----
    Les jetons correspondants doivent être stockés dans des
    fichiers ``jeton_api_ckan_[nom de l'attribut].txt`` dans
    le sous-répertoire ``jetons``. Il n'est pas problématique
    de ne pas en avoir pour tous les environnements, mais les
    fonctions de maintenance ne pourront évidemment pas être
    exécutées sans jeton.

    Les URL des instances sont en dur dans la fonction
    d'initialisation.

    ATTENTION : Pour l'heure, la vérification des
    certifications serveurs est inhibée (``verify=False``)
    sur les environnements de développement et pré-production,
    car les certificats utilisés sont auto-signés.
    
    """
    def __init__(self, intranet=False):
        # dev :
        p = Path(__path__[0]) / 'jetons' / 'jeton_api_ckan_dev.txt'
        api_token = None
        ...
        self.dev = CkanEnv(
            'dev',
            'développement',
            "https://dev.data.developpement-durable.gouv.fr",
            api_token=api_token,
            verify=False,
            intranet=intranet
            )
        # preprod :
        p = Path(__path__[0]) / 'jetons' / 'jeton_api_ckan_preprod.txt'
        api_token = None
        if p.exists() and p.is_file():
            with open(p, encoding='utf-8') as src:
                api_token = src.read()
        self.preprod = CkanEnv(
            'preprod',
            'pré-production',
        ...
        # prod :
        p = Path(__path__[0]) / 'jetons' / 'jeton_api_ckan_prod.txt'
        api_token = None
        if p.exists() and p.is_file():
            with open(p, encoding='utf-8') as src:
                api_token = src.read()
...
class CkanEnv:
    ...
    verify : bool
        La validité du certificat serveur doit-elle être contrôlée ?
    proxy : dict
        Dictionnaire des proxies à utiliser.
    ...
    def __init__(self, name, title, url, api_token=None, verify=True,
        ...
        self.verify = verify
        self.proxy = {
            'https': "http://proxy-rie.ac.i2:8080",
            'http': "http://proxy-rie.ac.i2:8080"
            } if intranet else None
    ...
    def refresh_orgs(self, verbose=True, strict=False):
        ...
        org_collection = json_import('organisations.json')
    ...
    def org_action(self, action, org_name, org_collection=None):
        ...
        if action in ('create', 'update', 'patch'):
            data_dict = org_collection[org_name].copy()
        else:
            data_dict = {}
    ...
    def action_request(self, api_action, data_dict=None):
        ...
        with warnings.catch_warnings():
            ...
            return requests.post(
                "{}/api/action/{}".format(self.url, api_action),
                json=data_dict,
    ...
    def harvest_sources_list(self, id_or_name='id'):
        ...
        while True:
            ...
            if not action_success(r):
                raise DialogError("Impossible de dresser la liste des" \
                    " moissonnages.")
    ...
    def clear_all_datasets(self, org_name=None, org_id=None,
        ...
        if org_name or org_id:
            ...
            r = ckan.dev.action_request('package_list')
            if not action_success(r):
                r_e = r_e or r
...
def json_import(filename):
    """Importe un fichier JSON du répertoire courant du module.
    ...
    """
    p = Path(__path__[0]) / filename
    if p.exists() and p.is_file():
        with open(p, encoding='utf-8') as src:
            d = json.load(src)
    else:
        raise FileNotFoundError("Fichier '{}' manquant dans " \
            "le répertoire courant.".format(filename))
    return d
